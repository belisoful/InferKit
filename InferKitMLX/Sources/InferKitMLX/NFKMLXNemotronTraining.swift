//
//  NFKMLXNemotronTraining.swift
//  InferKitMLX
//
//  Adapting a Nemotron-H decoder (Nemotron Nano 2) to a consumer's own text with LoRA. The objective is
//  causal language-model teacher forcing, as the reference trains: every position is scored on the next
//  token, so one forward pass scores a whole sequence. LoRA adapts the attention query and value
//  projections (the blocks Nemotron gives attention rather than a Mamba scan or a feed-forward);
//  everything else freezes.
//
//  The net builder, the single-file loader, and the `fineTune` recipe are the customization path the
//  parity rule requires. The causal-LM objective and the sparse-layer LoRA are shared in shape with the
//  Granite fine-tune; Nemotron's attention sits under the block's `mixer`, which is the only difference
//  in the LoRA target predicate.
//
...
/// The token-level cross-entropy a Nemotron-H fine-tune minimizes: each position predicts the next
/// token, matching `NemotronHForCausalLM`'s shifted loss.
///
/// Introduced in InferKit 0.4.0.
public struct NFKMLXNemotronObjective: Sendable {
    ...
    /// Scores a Nemotron network on one token sequence `[T]`: the decoder reads the whole sequence and
    /// every position is scored on the following token.
    public func callAsFunction(_ net: NFKMLXNemotronHNet, _ tokens: MLXArray) -> MLXArray {
...
public extension NFKMLXNemotronH {
    ...
    /// Builds a Nemotron-H network for fine-tuning or reloading a fine-tuned checkpoint. With a
    /// `weightsURL` the single-file checkpoint loads; without one the net is randomly initialized.
    static func network(weightsURL: URL?, configuration: NFKMLXNemotronHConfiguration) throws
        -> NFKMLXNemotronHNet {
    ...
    static func loadWeights(into net: NFKMLXNemotronHNet, from url: URL) throws {
    ...
    /// The projections LoRA adapts: the query and value of every attention block. Nemotron gives only a
    /// minority of blocks attention (the rest are Mamba scans or feed-forwards), and its attention sits
    /// under the block's `mixer`, so only those blocks carry a `q_proj` / `v_proj`. `NFKMLXLoRA` reaches
    /// this sparse subset of the layer array through its per-owner fallback.
    static func isAttentionProjection(_ path: String) -> Bool {
        path.hasSuffix(".q_proj") || path.hasSuffix(".v_proj")
    }

    /// Adapts a Nemotron-H network's attention with LoRA and trains it on token sequences.
    ///
    /// - Parameters:
    ...
    /// feed-forwards stay frozen. Call `NFKMLXLoRA.merge(into:)` before saving, so the result is one
    /// checkpoint ``loadWeights(into:from:)`` reads back. A run is minutes; call it off the render thread.
    @discardableResult
    static func fineTune(
        _ net: NFKMLXNemotronHNet,
        examples: (Int) -> MLXArray,
        rank: Int? = 8,
        alpha: Float = 16,
        objective: NFKMLXNemotronObjective = NFKMLXNemotronObjective(),
