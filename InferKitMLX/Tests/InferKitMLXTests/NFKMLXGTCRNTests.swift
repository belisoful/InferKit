...
//  InferKitMLXTests
//
//  GTCRN real-time speech enhancement (SCAFFOLD). The module layout and forward evaluate MLX
//  arrays, so they skip without a Metal library for MLX (see Tools/mlx-metallib.sh). The
//  reference-parity test is gated on the released weights and the recorded oracle output; it skips
//  until those are present (`IK_PARITY_GTCRN` + `IK_VAL_GTCRN`). Finalize it on the M1 Max against
//  `run_reference.py gtcrn`.
//
...
final class NFKMLXGTCRNTests: XCTestCase {
    ...
    func testReferenceParityOnTheReleasedWeights() throws {
        ...
        print("VALIDATION PARITY gtcrn: enhanced waveform cosine \(similarity)")
