...
//  InferKitMLXTests
//
//  The BasicVSR video super-resolution network. The forward, the bidirectional propagation, the
//  flow warp, and the weight round-trip evaluate MLX arrays, so they skip without a Metal library
//  for MLX (see Tools/mlx-metallib.sh).
//
