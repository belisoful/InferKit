...
try:
    ...
    # `laplacian` is used only on the training-only out_ref path; the eval oracle never calls it.
    def laplacian(*args, **kwargs):
        raise RuntimeError("laplacian stub: training-only path, not reachable at eval")
