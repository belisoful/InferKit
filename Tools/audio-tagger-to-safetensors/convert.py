#!/usr/bin/env python3
"""Convert a PANNs audio-tagging checkpoint to safetensors for InferKitMLX.
...
InferKitMLX's `NFKMLXAudioTagger` loads a safetensors checkpoint; MLX's `loadArrays` reads
...
The reference PANNs checkpoint (Cnn14) stores convolution blocks under `conv_block1.conv1.*` with batch
norm, plus `fc1` and `fc_audioset`. InferKitMLX's `NFKMLXAudioTaggerNet` groups these into `convs.N`
and a single `fc`, and pools globally at the end rather than between blocks, so the names do not line
up one-to-one. Aligning them (and the log-mel front-end) is a validation-sweep task; pass `--list-keys`
to print the checkpoint's names.

Usage:
    python convert.py Cnn14_mAP=0.431.pth --list-keys
    python convert.py Cnn14.pth audio-tagger.safetensors        # after a name remap is worked out

Get the weights from the PANNs release:
...
def extract_state_dict(checkpoint):
    if isinstance(checkpoint, dict):
        ...
        if checkpoint and all(isinstance(v, torch.Tensor) for v in checkpoint.values()):
            return checkpoint
    raise SystemExit("unrecognized checkpoint: expected a PANNs state dict")


def main():
    parser = argparse.ArgumentParser(description=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter)
    parser.add_argument("input", help="path to a PANNs .pth")
