...
- **[Matting & faces](Docs/model-index.md#matting-segmentation-and-faces)** — U²-Net, IS-Net, BiRefNet, Robust Video Matting (MobileNetV3 / ResNet-50), MODNet, SAM (ViT-B/L/H), SAM 2 and SAM 2.1 (all four Hiera sizes, video tracking), SAM 3 / 3.1 (text-prompted instance segmentation), RetinaFace, face alignment
- **[Detection & pose](Docs/model-index.md#detection-and-pose)** — YOLOv8 (n–x), YOLOv9 / v10 / 11 / v12 / 26 (every released size), RT-DETR and RT-DETRv2 (r18–r101), RF-DETR (nano–large), Table Transformer (Microsoft, table detection and structure recognition), SimpleBaseline pose, ViTPose (base-simple, base)
- **[Embeddings & vision-language](Docs/model-index.md#embeddings-reranking-and-vision-language)** — CLIP (ViT-B/32, B/16, L/14, L/14@336, and MetaCLIP weights), SigLIP 2 (all fourteen fixed-resolution sizes), Qwen3-Embedding, EmbeddingGemma, ModernBERT reranker, Laya (typed decisions: the open Jev, three variants), open-jev-deberta and Open-Jev 2B / 9B / 27B (community typed-decision reproductions), Qwen3-VL-Embedding and Qwen3-VL-Reranker (2B, text + image), SmolVLM2 (256M, 500M, 2.2B), Gemma 3 (4B, image + text), Gemma 3n (image + audio + text), Qwen3-VL, Pixtral 12B (Mistral, vision + text), Florence-2 (Microsoft, unified vision: detection, region captioning, grounding), TrOCR (Microsoft, handwriting-line reader), Sa2VA (ByteDance, segmentation VLM: referring segmentation through SAM 2), Phi-4-multimodal (Microsoft, image + speech + text in one decoder), Gemma 4 vision / audio
- **[Time series](Docs/model-index.md#embeddings-reranking-and-vision-language)** — Chronos-Bolt (Amazon, patched-T5 quantile forecasting)
- **[Language models](Docs/model-index.md#language-models)** — Qwen3 (dense, MoE), Qwen2-MoE, Mixtral, gpt-oss (MXFP4), GGUF, Qwen3.5 / 3.8, Qwen3.8-Flash-Next, Gemma 3 (270M, 1B, 4B), Gemma 3n (E2B, E4B), Gemma 4 (E2B, E4B, 26B-A4B, 12B), Gemma 2, DeepSeek V4 / V4.1, Codestral-Mamba (Mamba-2 SSM), Granite 4.0-H (hybrid Mamba/attention, dense and MoE), Nemotron Nano 2 (hybrid Mamba/attention/MLP), T5 / umT5, chat templates, constrained decoding
- **[Translation](Docs/model-index.md#translation)** — OPUS-MT (Marian, any Helsinki-NLP pair), M2M-100 (418M, 1.2B) and SMaLL-100, MADLAD-400 3B-MT (T5, 400+ languages), TranslateGemma (4B / 12B / 27B, Gemma 3 with the translation template)
- **[Video](Docs/model-index.md#video)** — RIFE HDv3 / v4, RAFT, BasicVSR, SD ×4 upscaler, V-JEPA 2 (Meta, self-supervised video feature extraction), Cosmos Tokenizer (NVIDIA, image and causal video tokenizers: continuous latents or discrete tokens, and back), the clip backend
- **[Audio](Docs/model-index.md#audio)** — Whisper (every size, tiny–large-v3-turbo), Parakeet-TDT, Granite Speech 3.3-2b (speech LLM: Conformer + Q-former + Granite decoder), Voxtral-Mini 3B (speech LLM: Whisper encoder + Llama decoder), Canary-1B-v2 (multitask ASR/translation: FastConformer encoder + attention encoder-decoder), Demucs v2 / HT Demucs, speech denoiser, MP-SENet, GTCRN, SGMSE+, StoRM, MossFormer2 SE / SR, DeepFilterNet3, VoiceRestore, Resemble Enhance, MetricGAN+, CMGAN, FRCRN, NU-Wave 2, Apollo, Conv-TasNet, MarbleNet and Silero VAD, PANNs tagger, DAC, SNAC (24 kHz speech, 32 / 44.1 kHz music), BigVGAN v2 vocoder, Mimi codec, Basic Pitch, hFT-Transformer and MuScriptor (audio to MIDI), All-In-One (beats, downbeats, tempo, sections)
...
- [Text → image & video](Docs/model-index.md#text-to-image-and-video) — Stable Diffusion 1.5 / 2.1 / SDXL-Turbo, Stable Diffusion 3 / 3.5, FLUX.1, FLUX.2, SD3 & FLUX ControlNet, IP-Adapter, Z-Image, SANA, Qwen-Image 2.1, LTX-Video, LTX-2, Wan, Wan 2.2 Animate
...
build and the output. A static consumer's binary grows by about 29 MB plus a 3.7 MB Metal library. A
dynamic consumer ships a 21 MB framework binary with the Metal library inside it.
...
matrix. The artifacts are not committed; they compress to about 64 MB across three release assets.

Prebuilt binaries for the latest release (v0.3.1):

- [`InferKit.xcframework.zip`](https://github.com/belisoful/InferKit/releases/download/v0.3.1/InferKit.xcframework.zip) (1.3 MB) — the core alone, no MLX
- [`InferKitMLX.xcframework.zip`](https://github.com/belisoful/InferKit/releases/download/v0.3.1/InferKitMLX.xcframework.zip) (38 MB) — static, core + MLX together, the best fit for most Objective-C apps
- [`InferKitMLXDynamic.xcframework.zip`](https://github.com/belisoful/InferKit/releases/download/v0.3.1/InferKitMLXDynamic.xcframework.zip) (25 MB) — one shared framework across several targets or plug-ins
